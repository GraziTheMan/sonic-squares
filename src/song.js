...
// timeline, merging tied melody runs and skipping muted rows.

// segments: [{ grid, tieGrid, drumGrid, steps }]
// Returns { melody: [{ step, durSteps, row, value }],
//           drums:  [{ step, row, value }], totalSteps }
export function collectSong(segments, { melodyAudible, drumAudible }) {
  const melody = [];
  const drums = [];
  let offset = 0;
  for (const seg of segments) {
    const { grid, tieGrid, drumGrid, steps } = seg;
    for (let row = 0; row < grid.length; row++) {
      ...
      while (step < steps) {
        ...
        melody.push({ step: offset + step, durSteps: end - step + 1, row, value: grid[row][step] });
        step = end + 1;
      }
    }
